#' \code{leverage} test of Linear model for Designed Multivariate Experiments objects
#' 
...
#' @param comps a numeric vector indicating the PCA component indexes to keep. Default the first two components (1:2)
...
#'  \item{over}{logical indicating if the leverage > quantile(leverage,level) for the given decomposed term}
#'
#' @seealso \code{\link{prcomp}}, \code{\link{quantile}}
#'
#' @author Cristobal Fresno and Elmer A Fernandez
#'
#' @references Tarazona S, Prado-Lopez S, Dopazo J, Ferrer A, Conesa A, Variable Selection for Multifactorial Genomic Data, Chemometrics and Intelligent Laboratory Systems, 110:113-122 (2012)  
#'
#' @examples
#' data(stemHypoxia)
...
setGeneric(name="leverage",def = function(object, comps=1:2, term, level=0.95){standardGeneric("leverage")})
#'
#' @name leverage
#' @rdname lmDME-leverage
#' @inheritParams leverage
#' @aliases leverage,lmDME-method
setMethod(f="leverage",signature = "lmDME", definition = function(object, comps=1:2, term, level=0.95){
  ##Check for term presence
  if(missing(term)){stop("ERROR: missing term")}
