...
#'   \item Fast limma based implementation based on lmFit,
#'   \item p and F-values over deflacted coefficients, 
#'   \item ploting functions for PCA and PLS.
#' }
#'
#' @section Slots: 
#' \itemize{
...
#'  \item modelDecomposition: list with the model formula obtained for each deflation step.
#'  \item residuals: list of residual matrices G rows(genes) x N columns(arrays-designed meassurements).
#'  \item coefficients: list of coefficient matrices. Each matrix will have G rows(genes) x k columns(levels of the corresponding model term).
#'  \item p.values: list of p-value matrices.
#'  \item F.p.values: list with corresponding F-p-values vectors for each individual.
#'  \item components: list with corresponding PCA or PLS components for the selected term/s.
#'  \item componentsType: name character vector to keep process trace of the variance/covariance components slot: decomposition ("pca" or "pls"), type ("apca" for ANOVA-PCA or "asca" for ANOVA-SCA) and scale ("none", "row" or "column") 
#' }
#'
...
#'  \item{design, model, modelDecomposition, residuals, coefficients, p.values, F.p.values, components and componentsType}{Getters for their respective slots.}
#' }
#'
#' @section ANOVA-linear-decomposition-functions:
#' \describe{
#'  \item{lmdme}{Function that produce the complete ANOVA decomposition based on model specification through formula interface. Technicaly is a high level wrapper of initialize function.}
...
#'  \item{Fpvalues, pvalues}{Getters for corresponding model decomposed asociated coefficient statistics in each step, for each observation}
#'  \item{residuals, resid, coef, coefficients, fitted.values, fitted}{Getters for corresponding model decomposed in each step}
#'  \item{permutation}{Produces de specified lmDME in addition to the requied permuted objects (sampling the columns of data), usign the same parameters to fit the model.}
#' }
#'
#' @section variance-covariance-decomposition-functions:
#' \describe{
#'  \item{decomposition}{Function to perform PCA or PLS over the ANOVA deflacted terms. PCA can be performed over E1, E2 or E3 and it is refered as ANOVA-PCA (APCA) but, if it is performed over the coefficients it is refered as ANOVA-SCA (ASCA). On the other hand PLSR is based on pls library and if it is performed on coefficients (ASCA like) it uses the identity matrix for output co-variance maximization or can be carried out over the E1, E2 or E3 (APCA like) using the design matrix as output.}
...
#'  \item{loadingplot}{Loafingplot for PCA interaction terms.}
#' }
#'
#' @author Cristobal Fresno and Elmer A Fernandez
#' 
#' @seealso \code{\link{lmdme}}, \code{\link{decomposition}}, \code{\link{biplot}}, \code{\link{loadingplot}} and additional related lmDME class functions.
#'
#' @references 
#' \enumerate{
...
#'  \item Zwanenburg G, Hoefsloot HCJ, Westerhuis JA, Jansen JJ, Smilde AK (2011) ANOVA.Principal component analysis and ANOVA-simultaneaus component analysis: a comparison J. Chemometrivcs 25:561-567 DOI:10.1002/cem.1400
#'  \item Tarazona S, Prado-Lopez S, Dopazo J, Ferrer A, Conesa A (2012) Variable Selection for Multifactorial Genomic Data, Chemometrics and Intelligent Laboratory Systems, 110:113-122
#' }
#'
#' @name lmDME-class
#' @rdname lmDME-Class
...
setClass(Class="lmDME",
  ...
  validity=function(object){
    ...
    modelsLength <- c(length(object@residuals), length(object@coefficients), length(object@p.values),
		      length(object@F.p.values),length(object@modelDecomposition))
    if(any(modelsLength != modelsLength[1])) stop("List type slots do not have the appropiate length")
    return(TRUE)
  }
)
