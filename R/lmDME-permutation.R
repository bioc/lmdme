#' \code{permutation} of the specified Linear Model for Designed Multivariate Experiment (lmDME)
#'
...
#' @param NPermutations number of permutations to be calculated. Default value is 100.
#' @param nCpus number of cores to be used. Default value is 1, i.e. sequencial calculation.
#'
#' @return 
#'  \item{list}{contains the original lmDME object plus the required amount of permuted versions} 
#'
...
setGeneric(name="permutation",def = function(model,data,design,Bayes=FALSE,verbose=FALSE,NPermutations=100,nCpus=1,...){standardGeneric("permutation")})
#'
#' @name permutation
#' @rdname lmDME-permutation
...
setMethod(f="permutation",signature=signature(model="formula",data="data.frame",design="data.frame"), definition = function(model,data,design,Bayes=FALSE,verbose=FALSE,NPermutations=100,nCpus=1,...){
  ##Generate the permutated samples: original structure + NPermutations
  permutations <- cbind(1:ncol(data),
			sapply(1:NPermutations,function(iteration,indexes){sample(indexes)}, indexes=1:ncol(data)))
  ...
  permutedModels  <- parlapply(permutedModels, function(index){
			...
			return(lmdme(model=model, data=data[,permutations[,index]], design=design, Bayes=Bayes, verbose=verbose, ...))},
		      mc.cores = nCpus)
  timeEnd <- Sys.time()
  printnow("End Time", as.character(timeEnd),"\n")
  timedifference <- timeEnd-time
  printnow("Time difference of ", format(unclass(timedifference ), digits = getOption("digits")), " ", attr(timedifference , "units"), "\n")
